...
export default function ServiceBox({ slug, name, description, isCore = false }: ServiceBoxProps) {
    return (
        <Link href={`/suite/services/${slug}`} className={`group block h-full bg-black border border-white/10 p-6 hover:border-neon transition-all duration-300 relative overflow-hidden ${isCore ? 'col-span-1 md:col-span-2 row-span-2' : ''}`}>
